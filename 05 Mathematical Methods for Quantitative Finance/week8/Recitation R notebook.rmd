...
dataurl <- "https://courses.edx.org/asset-v1:MITx+15.455x+3T2020+type@asset+block@michaud.csv"
...
###plot(Sigma,Mu);grid()
ggplot(X,aes(x=SD,y=Mean)) + geom_point(color="red",size=2) + labs(title="Monthly Return vs. Risk for Individual Assets", x="Sigma", y="Mu") + geom_label_repel(aes(label=X$...1),vjust=1)
```
...
# taking linear combinations of those two portfolios, we can reach everything on the efficient frontier
...
# varying mu all the way up to our maximum value of mu
...
# 100 points on the efficient frontier
# numerical result of quadratic programming
Npoints <- 100
...
This is the traditional MVO plot, showing portfolios in $n$ dimensional asset space (The red dots are basis vectors) projected down to their values in the two-dimensional space of risk and return.
...
# from lowest return up to the highest return
...
# So the blue line is our unconstrained efficient frontier.
# The black line that's the envelope curve of all these red dots, that's the true efficient frontier under the constraints with the inequality constraints we've been talking about.
# each line of red dots that you see is the output of one of our 100 simulations
```
